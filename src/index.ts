...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Climatiq');
}
...
async function searchFactors(apiKey: string, args: Record<string, unknown>) {
  const params = new URLSearchParams({
    ...
    // Climatiq now REQUIRES data_version (was optional) — 400 "missing field
    // `data_version`" otherwise. Caret ^34 = latest compatible in release 34
    // (current latest); callers may override for repeatable/pinned results.
    data_version: typeof args.data_version === 'string' && args.data_version.trim() ? String(args.data_version).trim() : '^34',
  });
